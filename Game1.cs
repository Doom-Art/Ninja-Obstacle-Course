...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        private bool _teacherMode, _devil;
        ...
        protected override void Initialize()
        {
                ...
                foreach (string line in File.ReadLines(@"Save.txt", Encoding.UTF8))
                {
                    switch (counter)
                    {
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            _devil = line == "DevilSoul";
                            break;
                        case 9:
                            _player.AutoSprint = line == "True";
                            break;
                        case 10:
                            _soundOn = line == "True";
                            break;
            ...
            //Load High Scores
            _highScores = new Score[_levels.Count, 9];
            if (File.Exists("HighScores.txt"))
            {
                int level = 0, counter = 0;
                foreach (string line in File.ReadLines(@"HighScores.txt"))
                {
                    _highScores[level, counter] = Score.LoadSave(line);
                    counter++;
                    if (counter == 9)
                    {
                        level++;
            ...
            //Set all Empty High Scores to default
            for (int i = 0; i < _levels.Count; i++)
            {
                for (int j = 0; j < 9; j++)
                    if (_highScores[i, j] == null)
                    {
                        int difficulty;
                        if (j < 3)
                            difficulty = 3;
                            ...
                            difficulty = 1;
                        _highScores[i, j] = Score.Default(difficulty);
                    }
            }
        }
        ...
        protected override void LoadContent()
        {
            ...
            _powerups = new()
            {
                ...
                new Powerup("Limiter Removal", "Removes all \nthose annoying \nLimits to bring \nMages to the \nnext difficulty", 10)
                {
                    LimiterRemoval = true
                },

                new Powerup("Deal With The Devil", "Sell your soul to \nthe Devil to \nunlock some skins. \nPowerups will \ndissapear on death \nafter buying this", 0)
            };
            ...
            _devilBG = Content.Load<Texture2D>("Background Pictures/Devil");
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (screen == Screen.Game)
            {
                ...
                if (!_p1Death)
                {
                    ...
                    {
                        if (_levels[_cL].DidPlayerDie(_player) || (_currentPowerUp == _powerups.Count - 1))
                        {
                            _p1Death = true;
                        }
                    }
            ...
            else if (screen == Screen.HighScore)
            {
                if (_newHighScore)
                {
                ...
                if (_continue.Update(_mouseState))
                {
                    if (_newHighScore)
                    {
                        if (_name == "")
                    ...
                    if (_levels.Count > _cL + 1)
                    {
                        ...
                        _skinInLevel = _levels[_cL].SetDefaults(_player, _difficulty, _ninjaSkins, _cL);
                        _seconds = 0;
                        SaveGame(_coins, _deathCounter, _ninjaSkins, _teacherMode, _equipedPet, _pets, _currentPowerUp, _bgColor, _devil, _player.AutoSprint, _soundOn, _highScores);
                        screen = Screen.Game;
                    }
                    else
                    {
            ...
            else if (screen == Screen.Death)
            {
                if (_soundOn)
                    _deathSound.Play();
                _playerRespawnTimer += (float)gameTime.ElapsedGameTime.TotalSeconds;
                if (_currentPowerUp == _powerups.Count - 1)
                    _devil = true;
                if (_playerRespawnTimer > 4)
                {
                    ...
                    if (_currentPowerUp != -1)
                        if (_powerups[_currentPowerUp].OneTimeUse)
                            _currentPowerUp = -1;
                    screen = Screen.Game;
                    _p1Death = false;
                    _graphics.PreferredBackBufferWidth = 900;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            else if (screen == Screen.Death)
            {
                ...
                _spriteBatch.Begin();
                if (_devil)
                    _spriteBatch.Draw(_devilBG, new Vector2(0, 0), Color.White);
                else
            ...
            else if (screen == Screen.HighScore)
            {
                GraphicsDevice.Clear(Color.Aqua);
                _spriteBatch.Begin();
                _spriteBatch.DrawString(_highScoreFont, "1._" + _highScores[_cL, 0].ToString(), new Vector2(10, 0), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "2._" + _highScores[_cL, 1].ToString(), new Vector2(10, 40), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "3._" + _highScores[_cL, 2].ToString(), new Vector2(10, 80), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "1._" + _highScores[_cL, 3].ToString(), new Vector2(10, 130), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "2._" + _highScores[_cL, 4].ToString(), new Vector2(10, 170), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "3._" + _highScores[_cL, 5].ToString(), new Vector2(10, 210), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "1._" + _highScores[_cL, 6].ToString(), new Vector2(10, 260), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "2._" + _highScores[_cL, 7].ToString(), new Vector2(10, 300), Color.Black);
                _spriteBatch.DrawString(_highScoreFont, "3._" + _highScores[_cL, 8].ToString(), new Vector2(10, 340), Color.Black);
                if (_newHighScore)
                {
                    _spriteBatch.DrawString(_highScoreFont, $"Please_input_your_name_to_save_the_new_high_score: {_name}", new Vector2(10,400), Color.Black);
                }
        ...
        public static void SaveGame(int coins, int death, List<Skin> ninjaSkins, bool teacherMode, int equippedPet, List<Pet> pets, int currentPU, int bgColor, bool devil, bool autoSprint, bool soundOn, Score[,] highscores)
        {
            ...
            if (devil)
                save.WriteLine("DevilSoul");
            else
                save.WriteLine("False");
            save.WriteLine(autoSprint);
            save.WriteLine(soundOn);
            save.Close();
            StreamWriter saveScores = new("HighScores.txt");
            foreach (Score s in highscores)
            {
                saveScores.WriteLine(s.Save());
            }
            saveScores.Close();
        }
    }
